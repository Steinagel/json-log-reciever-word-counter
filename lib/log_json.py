...
def readJSONlog(logfile, current_data, filterfunction = (lambda x: True), customjson = None):
    ...
    """
    JSONrecords = current_data
    for x in logfile:
        #if the record in the logfile returns true from the filter function convert it to JSON and add it the records to return
        _rec = loads(x[:-1],object_hook = customjson)
        # if filterfunction(rec): JSONrecords.append(rec)
        if filterfunction(_rec): 
            rec = {
            "object": _rec["file_processed"],
            "data":{
                    "start_count": int(_rec["count"]),
                    "end_count": 0,
                    "total_calls": 0,
                    "first_time": _rec["asctime"],
                    "last_time": "",
                    "total_time": 0,
                    "message": _rec["message"],
                    "module_name": _rec["name"],
                    "levelname": _rec["levelname"]
                }
            }
            index = None
            for _index, item in enumerate(JSONrecords):
                if(_rec["file_processed"] == item["object"]):
                    index = _index
                    JSONrecords[index]["data"]["end_count"]=JSONrecords[index]["data"]["end_count"]+int(_rec["count"])
